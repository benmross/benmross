...
function Architecture() {
  ...
  const wire = (d: string, label: string, lx: number, ly: number, dashed = false, i = 0) => (
    <g>
      ...
      />
      <m.text x={lx} y={ly} fill="#8d8b84" fontSize="11.5" initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : undefined} transition={{ delay: 1 + i * 0.15 }}>
        {label}
      </m.text>
    </g>
  )
...
const CARDS: { kind: 'img' | 'arch'; src?: string; w: number; alt?: string; cap?: string; contain?: boolean }[] = [
          ...
          {CARDS.map((c, i) => (
            <figure key={i} className="kart-card" style={{ ['--w' as string]: c.w }}>
              <div className={`kart-frame${c.contain ? ' contain' : ''}`}>
                {c.kind === 'img' ? <img {...img(c.src!, '(max-width: 820px) 70vw, 45vw')} alt={c.alt} loading="lazy" decoding="async" /> : <Architecture />}
              </div>
